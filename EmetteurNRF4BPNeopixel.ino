...
#define N_LEDS 4             // Utilisation de 4 NeoPixels
...
typedef struct {
  bool Msg1;  // État de LED1
  bool Msg2;  // État de LED2
  bool Msg3;  // État de LED3
  bool Msg4;  // État de LED4
} MsgSt;
...
void loop() {
  ...
  if (reponseFromRecepteur.ledState2) {
      strip.setPixelColor(1, strip.Color(0, 255, 0));  // LED2 en vert si allumée
  } else {
    strip.setPixelColor(1, strip.Color(0, 0, 0));    // LED2 éteinte
  }

  if (reponseFromRecepteur.ledState3) {
      strip.setPixelColor(2, strip.Color(0, 0, 255));  // LED3 en bleu si allumée
  } else {
    strip.setPixelColor(2, strip.Color(0, 0, 0));    // LED3 éteinte
  }

  if (reponseFromRecepteur.ledState4) {
      strip.setPixelColor(3, strip.Color(255, 255, 255));  // LED4 en blanc si allumée
  } else {
    strip.setPixelColor(3, strip.Color(0, 0, 0));      // LED4 éteinte
  }
